...
void attacker_config_page_table(void)
{
    ...
    SGX_ASSERT(get_ecall_profile_aex(eid, &code_address));
...
int main(int argc, char **argv)
{
    ...
    start = rdtsc_begin();
    SGX_ASSERT(ecall_profile_aex(eid));
    end = rdtsc_end();
    info("NOINT: %d", end - start);
    ...
    info("INT-%d: %d", NUM_RUNS, end - start);
